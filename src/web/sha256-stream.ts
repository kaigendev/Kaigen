...
/** Small dependency-free incremental SHA-256 for streamed browser downloads. */
export class StreamingSha256 {
  ...
  update(input: Uint8Array) {
    ...
    if (this.bufferLength > 0) {
      const copied = Math.min(64 - this.bufferLength, input.length);
  ...
  private compress(block: Uint8Array) {
    ...
    for (let index = 16; index < 64; index += 1) {
      const left = this.schedule[index - 15];
      const right = this.schedule[index - 2];
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first = (h + sigma1 + choice + ROUND_CONSTANTS[index] + this.schedule[index]) >>> 0;
