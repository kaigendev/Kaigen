...
- Tor выключен по умолчанию и запускается только после явного включения пользователем. Приложение запускает только вложенный `TorExpertBundle\tor\tor.exe`; системный Tor и Tor Browser не используются. Если сеть блокирует обычный Tor, для подключения можно выбрать встроенный transport `obfs4`; прямой режим остаётся отдельным рабочим маршрутом без Tor и прокси.
